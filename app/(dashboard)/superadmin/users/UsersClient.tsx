...
export default function UsersClient() {
    ...
    const [editingInsuranceAccess, setEditingInsuranceAccess] = useState<string[]>(['personal', 'commercial'])
        ...
        try {
            ...
            const res = await fetch('/api/superadmin/users', {
                ...
                body: JSON.stringify({ 
                    id, 
                    role: editingRole,
                    insurance_access: editingRole === 'csr' ? editingInsuranceAccess : undefined
                })
            })
            const j = await res.json()
            if (j.error) throw new Error(j.error)
                                    ...
                                    {editingUserId === user.id ? (
                                        <div className="w-full space-y-2">
                                            <select
                                                ...
                                                className="bg-white border-2 border-indigo-400 rounded-lg p-1.5 text-xs font-bold focus:ring-2 focus:ring-indigo-500/20 outline-none w-full"
                                            >
                                            ...
                                            {editingRole === 'csr' && (
                                                <div className="flex items-center gap-3 bg-gray-50 border border-gray-200 p-2 rounded-lg mt-1">
                                                    <label className="flex items-center gap-1.5 text-xs font-bold text-gray-700 cursor-pointer">
                                                        <input
                                                            type="checkbox"
                                                            checked={editingInsuranceAccess.includes('personal')}
                                                            onChange={e => setEditingInsuranceAccess(e.target.checked ? [...editingInsuranceAccess, 'personal'] : editingInsuranceAccess.filter(a => a !== 'personal'))}
                                                    ...
                                                    <label className="flex items-center gap-1.5 text-xs font-bold text-gray-700 cursor-pointer">
                                                        <input
                                                            type="checkbox"
                                                            checked={editingInsuranceAccess.includes('commercial')}
                                                            onChange={e => setEditingInsuranceAccess(e.target.checked ? [...editingInsuranceAccess, 'commercial'] : editingInsuranceAccess.filter(a => a !== 'commercial'))}
                                                            className="w-3.5 h-3.5 text-indigo-600 rounded border-gray-300"
                                                        />
                                                        Commercial
                                                    </label>
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {user.role === 'superadmin' && <ShieldAlert size={14} className="text-purple-600 flex-shrink-0" />}
                                            </div>
                                            {user.role === 'csr' && user.insurance_access && (
                                                <div className="flex gap-1 flex-wrap">
                                                    {user.insurance_access.includes('personal') && (
                                                        <span className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded text-[9px] font-bold uppercase">Personal</span>
                                                    )}
                                                    {user.insurance_access.includes('commercial') && (
                                                        <span className="px-2 py-0.5 bg-blue-50 text-blue-700 border border-blue-100 rounded text-[9px] font-bold uppercase">Commercial</span>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                                ...
                                <div className="flex justify-end items-center gap-2 pt-1">
                                    {editingUserId === user.id ? (
                                        <div className="flex justify-end gap-2 w-full">
                                            <button onClick={() => setEditingUserId(null)} className="px-3 py-1.5 flex-1 justify-center text-xs text-white bg-rose-600 hover:bg-rose-700 border border-rose-600 rounded-lg transition font-medium flex items-center gap-1 shadow-sm">
                                                <X size={14} /> Cancel
                                            </button>
                                            <button onClick={() => handleUpdateRole(user.id, user.role)} className="px-3 py-1.5 flex-1 justify-center text-xs text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition font-medium flex items-center gap-1 shadow-sm">
                                                <Save size={16} /> Save
                                            </button>
                                        </div>
                                    ) : (
                                        <div className="flex justify-end gap-1.5 w-full">
                                            <button onClick={() => { 
                                                ...
                                                setEditingInsuranceAccess(user.insurance_access || ['personal', 'commercial']);
                                            }} className="flex-1 flex items-center justify-center gap-1 px-3 py-1.5 text-xs text-emerald-600 font-medium bg-emerald-50 hover:bg-emerald-100 rounded-lg transition border border-emerald-100" title="Edit Access Role">
                                                <Edit2 size={15} /> Edit Role
                                            </button>
                                            <button onClick={() => handleDelete(user.id)} className="px-3 py-1.5 text-xs text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition flex items-center justify-center gap-1 border border-red-100" title="Delete User">
                                                <Trash2 size={15} />
                                            </button>
                                        </div>
                                    )}
                                        ...
                                        {editingUserId === user.id ? (
                                            <div className="space-y-2">
                                                <select
                                                    value={editingRole}
                                                    onChange={e => setEditingRole(e.target.value)}
                                                    className="bg-white border-2 border-indigo-400 rounded-xl p-2 text-xs font-bold focus:ring-2 focus:ring-indigo-500/20 outline-none w-full max-w-[180px]"
                                                >
                                                    <option value="csr">CSR</option>
                                                    <option value="admin">Admin</option>
                                                    <option value="accounting">Accounting</option>
                                        ...
                                        <div className="flex justify-end items-center gap-1.5">
                                            {editingUserId === user.id ? (
                                                <div className="flex justify-end gap-1.5">
                                                    <button onClick={() => setEditingUserId(null)} className="px-3 py-1.5 text-xs text-white bg-rose-600 hover:bg-rose-700 border border-rose-600 rounded-lg transition font-medium flex items-center gap-1 shadow-sm">
                                                        <X size={14} /> Cancel
                                                    </button>
                                                    <button onClick={() => handleUpdateRole(user.id, user.role)} className="px-3 py-1.5 text-xs text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition font-medium flex items-center gap-1 shadow-sm">
                                                        <Save size={16} /> Save
                                                    </button>
                                                </div>
                                            ) : (
                                                <div className="flex justify-end gap-1.5">
                                                    <button onClick={() => { 
                                                        ...
                                                        setEditingRole(user.role);
                                                        setEditingInsuranceAccess(user.insurance_access || ['personal', 'commercial']);
                                                    }} className="flex items-center gap-1 px-3 py-1.5 text-xs text-emerald-600 font-medium bg-emerald-50 hover:bg-emerald-100 rounded-lg transition border border-emerald-100" title="Edit Role">
                                                        <Edit2 size={15} /> Edit Role
                                                    </button>
                                                    <button onClick={() => handleDelete(user.id)} className="p-1.5 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition border border-red-100" title="Delete User">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                            )}
