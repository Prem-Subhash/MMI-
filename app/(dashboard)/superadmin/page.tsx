...
import {
    ...
    ArrowRight, Shield, Building2, TrendingUp,
    Clock, CheckCircle2, Home, AlertCircle
} from 'lucide-react'
...
export default async function SuperAdminDashboard() {
    ...
    const { data: mortgageLoans } = await supabase
        .from('mortgage_loans')
        .select('id, client_name, stage, loan_amount, expected_commission, created_at, pipeline_type')
        .order('created_at', { ascending: false })

    // Compute Headcounts
    const totalUsers = profilesList?.length || 0
    const insuranceUsersCount = profilesList?.filter(p => p.role === 'csr' || p.role === 'admin' || p.role === 'accounting' || p.portal_access?.includes('insurance')).length || 0
    const mortgageUsersCount = profilesList?.filter(p => p.role === 'mortgage' || p.portal_access?.includes('mortgage')).length || 0
    const lendingUsersCount = profilesList?.filter(p => p.role === 'lending' || p.role === 'accurate_lending' || p.portal_access?.includes('lending') || p.portal_access?.includes('accurate_lending')).length || 0
    ...
    const totalMortgageCommission = mortgageLoans?.reduce((sum, l) => sum + (Number(l.expected_commission) || 0), 0) || 0
    const totalMortgageVolume = mortgageLoans?.reduce((sum, l) => sum + (Number(l.loan_amount) || 0), 0) || 0
    ...
    const combinedActivity = [
        ...(leads?.slice(0, 15).map(l => ({
            ...
            amount: l.total_premium || l.new_premium ? formatCurrency(Number(l.total_premium || l.new_premium)) : 'Pending Quote',
        ...
        ...(mortgageLoans?.slice(0, 15).map(m => ({
            ...
            amount: m.loan_amount ? formatCurrency(Number(m.loan_amount)) : (m.expected_commission ? formatCurrency(Number(m.expected_commission)) : 'Pending Review'),
        ...
        <div className="p-4 sm:p-6 lg:p-8 space-y-10 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-slate-500 text-sm sm:text-base mt-1 max-w-3xl">
                        Comprehensive real-time oversight across Innovative Insurance, Moonstar Mortgage, and Accurate Lending.
                    </p>
                </div>
            </div>
                        ...
                        <Link key={i} href={stat.href} className="group">
                            ...
                            `}>
                                ...
                                <div className="flex items-center gap-2">
                                    <div className={`p-2 rounded-lg ${stat.iconBg} ${stat.hoverIconBg} transition-all duration-300 inline-flex group-hover:scale-110`}>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm flex flex-col justify-between hover:border-emerald-500/50 transition-all">
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="text-xs font-bold text-slate-400">{insuranceUsersCount} Staff</span>
                            </div>
                            <h3 className="text-xl font-bold text-slate-900 mb-4">Property & Casualty CRM</h3>
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between border-b border-gray-100 pb-2">
                                    <span className="text-slate-500">Total Active Leads</span>
                                    <span className="font-bold text-slate-800">{leads?.length || 0}</span>
                                </div>
                                <div className="flex justify-between border-b border-gray-100 pb-2">
                                    <span className="text-slate-500">Quotes in Progress</span>
                                    <span className="font-bold text-slate-800">{totalQuotesSent}</span>
                                </div>
                                <div className="flex justify-between pb-1">
                                    <span className="text-slate-500">Bound Premium Volume</span>
                                    <span className="font-bold text-emerald-600">{formatCurrency(totalBoundPremium)}</span>
                                </div>
                            </div>
                        </div>
                        <Link href="/superadmin/insurance/leads" className="mt-6 w-full py-2.5 px-4 bg-emerald-50 hover:bg-emerald-600 hover:text-white text-emerald-700 font-bold rounded-xl text-center text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5">
                    ...
                    {/* Moonstar Mortgage */}
                    <div className="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm flex flex-col justify-between hover:border-amber-500/50 transition-all">
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                <span className="px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-amber-100 text-amber-800">
                                    Moonstar Mortgage
                                </span>
                                <span className="text-xs font-bold text-slate-400">{mortgageUsersCount} Staff</span>
                            </div>
                            <h3 className="text-xl font-bold text-slate-900 mb-4">Residential Lending Portal</h3>
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between border-b border-gray-100 pb-2">
                                    <span className="text-slate-500">Total Applications</span>
                                    <span className="font-bold text-slate-800">{totalMortgageLoans}</span>
                                </div>
                                <div className="flex justify-between border-b border-gray-100 pb-2">
                                    <span className="text-slate-500">Total Loan Volume</span>
                                    <span className="font-bold text-slate-800">{formatCurrency(totalMortgageVolume)}</span>
                                </div>
                                <div className="flex justify-between pb-1">
                                    <span className="text-slate-500">Projected Commission</span>
                                    <span className="font-bold text-amber-600">{formatCurrency(totalMortgageCommission)}</span>
                                </div>
                            </div>
                        </div>
                        <Link href="/superadmin/mortgage/applications" className="mt-6 w-full py-2.5 px-4 bg-amber-50 hover:bg-amber-600 hover:text-white text-amber-700 font-bold rounded-xl text-center text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm flex flex-col justify-between hover:border-purple-500/50 transition-all relative overflow-hidden">
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <span className="text-xs font-bold text-purple-600 uppercase">UI Prototype</span>
                            </div>
                            <h3 className="text-xl font-bold text-slate-900 mb-4">Commercial Finance Portal</h3>
                            <div className="space-y-3 text-sm">
                                <div className="flex justify-between border-b border-gray-100 pb-2">
                                    <span className="text-slate-500">Commercial Pipeline (Preview)</span>
                                    <span className="font-bold text-slate-800">14 Inquiries</span>
                                </div>
                                <div className="flex justify-between border-b border-gray-100 pb-2">
                                    <span className="text-slate-500">Term Sheets Received</span>
                                    <span className="font-bold text-slate-800">5 ($6.4M Vol)</span>
                                </div>
                                <div className="flex justify-between pb-1">
                        ...
                        <Link href="/lending" className="mt-6 w-full py-2.5 px-4 bg-purple-50 hover:bg-purple-600 hover:text-white text-purple-700 font-bold rounded-xl text-center text-xs uppercase tracking-wider transition-all flex items-center justify-center gap-1.5">
                            <span>Open Commercial Prototype</span>
                            <ArrowRight size={14} />
                        </Link>
                    </div>
                </div>
            ...
            <div className="bg-white border border-gray-200/80 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-5 border-b border-gray-100 bg-slate-50/70 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">Chronological feed combining latest Insurance leads and Mortgage loan updates</p>
                    </div>
                </div>
                                ...
                                combinedActivity.map((item) => (
                                    <tr key={item.id} className="hover:bg-slate-50/70 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <Link href={item.href} className="text-xs font-bold text-blue-600 hover:text-blue-800 uppercase tracking-wide">
                                                View Details →
                                            </Link>
                                        </td>
                                    </tr>
            ...
            <div>
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="text-lg font-bold text-gray-800">Executive Quick Actions</h2>
                </div>
                        ...
                        <Link key={i} href={link.href} className="group">
                            ...
                            `}>
                                <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${link.accent}
                                    transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left rounded-t-2xl`}
                                />
                                <div className={`p-2.5 rounded-xl ${link.iconBg} ${link.hoverIcon} transition-all duration-300 inline-flex mb-3 group-hover:scale-110`}>
                                    {link.icon}
                                </div>
