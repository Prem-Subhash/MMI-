...
export default function PipelineView({
  ...
  // Dialog states
  const [selectedCreatePipeline, setSelectedCreatePipeline] =
    useState<PipelineType>(pipelineType);
  ...
  const pipelineStages = MORTGAGE_STAGES.filter(
  ...
  const fetchLoans = async (showToast = false) => {
    try {
      ...
      if (selectedOfficer !== "ALL")
        params.append("loan_officer_name", selectedOfficer);
      if (selectedProcessor !== "ALL")
        params.append("processor_name", selectedProcessor);
      if (selectedTxType !== "ALL")
        params.append("transaction_type", selectedTxType);
  ...
  const handleStageMove = async (
    ...
    try {
      ...
      if (res.ok && json.success) {
        setLoans((prev) =>
          prev.map((l) =>
            l.id === loan.id ? { ...l, stage: targetStageCode } : l,
          ),
        );
        ...
        toast(
          `Moved "${loan.client_name || "Borrower"}" to ${targetLabel}`,
          "success",
        );
      } else {
        toast(
          json.error || "Failed to move loan application to new stage.",
          "error",
        );
      }
    } catch (e: any) {
  ...
  const handleDelete = async (loan: MortgageLoan) => {
    if (
      !confirm(
        `Are you sure you want to delete loan application for "${loan.client_name}"?`,
      )
    ) {
      return;
    }
    try {
      ...
      if (res.ok) {
        ...
        toast(
          `Deleted application for "${loan.client_name || "Borrower"}"`,
          "success",
        );
      } else {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-5 shadow-sm space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap sm:flex-nowrap items-center gap-3 w-full sm:w-auto">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${loading ? "animate-spin text-[#10B889]" : ""}`}
              />
            </button>
        ...
        {loading ? (
          <div className="h-96 flex flex-col items-center justify-center text-gray-500 gap-3">
            ...
            <p className="text-sm font-semibold">
              Loading Moonstar Mortgage Pipeline...
            </p>
          </div>
        ) : error ? (
          ...
          <div className="h-96 flex flex-col items-center justify-center text-center p-8 bg-white border border-dashed border-gray-300 rounded-xl shadow-sm">
            ...
            <p className="text-xs text-gray-500 max-w-sm mt-1">
              No applications match your current filters. Click &quot;Create
              Application&quot; to add a new borrower.
            </p>
            <button
              type="button"
              onClick={() => {
      ...
      <div className="px-6 py-4 bg-white border border-gray-200 rounded-xl shadow-sm flex items-center justify-between text-xs text-gray-600">
        <div>
          Showing{" "}
          <span className="text-gray-900 font-bold">{loans.length}</span> of{" "}
          <span className="text-gray-900 font-bold">{totalRecords}</span>{" "}
          applications
        </div>
      ...
      <LoanFormModal
        ...
        defaultPipelineType={
          editingLoan ? editingLoan.pipeline_type : selectedCreatePipeline
        }
        onSuccess={(savedLoan) => {
          fetchLoans();
      ...
      <LoanDetailModal
        ...
        onMoveStage={(loan, newStageCode) =>
          handleStageMove(loan, newStageCode)
        }
      />
    </div>
  );
