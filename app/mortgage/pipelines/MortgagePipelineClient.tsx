...
export default function MortgagePipelineClient({
  initialLoans = [],
  isSuperAdmin = false,
}: MortgagePipelineClientProps) {
  const pipelines = [
    {
      id: "NEW_LOAN" as PipelineType,
      name: "New Loan Pipeline",
      description: "Full origination workflow from initial intake to audit",
    },
    {
      id: "PRE_APPROVAL" as PipelineType,
      name: "Pre-Approval Pipeline",
      description: "Borrower pre-qualification and manual underwriting",
    },
  ];

  const [selectedPipeline, setSelectedPipeline] =
    useState<PipelineType>("NEW_LOAN");
  ...
  const currentStages = MORTGAGE_STAGES.filter(
    (s) => s.pipeline === selectedPipeline,
  );

  // Filter loans belonging to selected pipeline
  const pipelineLoans = initialLoans.filter(
    (loan) => loan.pipeline_type === selectedPipeline,
  );
  ...
  return (
    <div className="w-full animate-fade-in pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-600 text-sm mt-1">
            Executive breakdown of active mortgage applications across
            origination and pre-approval stages.
          </p>
        </div>
        <Link
          href={
            isSuperAdmin ? "/superadmin/mortgage/applications" : "/mortgage"
          }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-8">
        {pipelines.map((p) => {
          const totalLoansInPipeline = initialLoans.filter(
            (l) => l.pipeline_type === p.id,
          ).length;
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  {p.id === "NEW_LOAN" ? (
                    <Layers
                      className={`w-5 h-5 ${isSelected ? "text-[#10B889]" : "text-gray-500"}`}
                    />
                  ) : (
                    <FileCheck
                      className={`w-5 h-5 ${isSelected ? "text-[#10B889]" : "text-gray-500"}`}
                  ...
                  <h3
                    className={`text-base sm:text-lg font-bold break-words ${isSelected ? "text-gray-900" : "text-gray-800"}`}
                  >
                    {p.name}
                  </h3>
                </div>
              ...
              <div className="mt-4 pt-3 border-t border-gray-100 flex justify-between items-center">
                ...
                <span
                  className={`text-2xl font-bold ${isSelected ? "text-[#10B889]" : "text-gray-700"}`}
                >
          ...
          {currentStages.map((s) => {
            ...
            return (
              ...
              >
                <span>{s.label}</span>
                <span
                  className={`px-1.5 py-0.5 rounded-full text-[10px] ${isActive ? "bg-white/20 text-white" : "bg-gray-100 text-gray-600"}`}
                >
                  {count}
                </span>
              </button>
          ...
          {filteredLoans.length === 0 ? (
            <div className="p-16 text-center text-gray-500 flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-gray-500 mt-1 max-w-xs">
                Select another stage or clear your search query to view
                applications.
              </p>
            </div>
          ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 sm:px-6 py-4 text-gray-600 align-top">
                          {formatPhoneNumber(loan.phone)}
                        </td>
                        <td className="px-4 sm:px-6 py-4 text-gray-600 break-all align-top">
                          {loan.email}
                        </td>
                        ...
                        <td className="px-4 sm:px-6 py-4 text-center align-top">
                          {(() => {
                            const targetRoute = isSuperAdmin
                              ? loan.pipeline_type === "PRE_APPROVAL"
                                ? "/superadmin/mortgage/pre-approval"
                                : "/superadmin/mortgage/new-loan"
                              : loan.pipeline_type === "PRE_APPROVAL"
                                ? "/mortgage/pipeline/pre-approval"
                                : "/mortgage/pipeline/new-loan";
                            return (
                              <Link
                                href={targetRoute}
...
function StageBadge({ stage }: { stage: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-full text-xs font-bold whitespace-nowrap shadow-2xs ${badgeClass}`}
    >
      {label}
    </span>
  );
