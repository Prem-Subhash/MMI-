'use client'

import { LayoutGrid, Users, Settings, GitBranch, RefreshCw, Briefcase, FileText, PieChart, ListTodo, DollarSign, Activity } from 'lucide-react'
import Link from 'next/link'
import { usePathname } from 'next/navigation'
import { useEffect, useState } from 'react'
import { supabase } from '@/lib/supabaseClient'

interface SidebarProps {
    setIsHovered: (hovered: boolean) => void
    isHovered: boolean
    isMobileOpen: boolean
    setIsMobileOpen: (open: boolean) => void
}

interface MenuItem {
    label: string
    href: string
    icon: React.ReactNode
}

export default function Sidebar({ setIsHovered, isHovered, isMobileOpen, setIsMobileOpen }: SidebarProps) {
    const pathname = usePathname()
    const [role, setRole] = useState<string | null>(null)

    useEffect(() => {
        const fetchRole = async () => {
            const { data: { user } } = await supabase.auth.getUser()
            if (user) {
                const { data } = await supabase
                    .from('profiles')
                    .select('role')
                    .eq('id', user.id)
                    .single()

                if (data?.role) {
                    setRole(data.role)
                }
            }
        }
        fetchRole()
    }, [])

    const isActive = (path: string) => {
        if (pathname === path) return true
        if (path !== '/' && path !== `/${role}` && pathname.startsWith(path)) return true
        return false
    }

    const csrMenu: MenuItem[] = [
        { label: 'Dashboard', href: '/csr', icon: <LayoutGrid size={24} /> },
        { label: 'Personal Pipeline', href: '/csr/pipeline/personal', icon: <GitBranch size={24} /> },
        { label: 'Personal Renewal', href: '/csr/renewals/personal', icon: <RefreshCw size={24} /> },
        { label: 'Commercial Pipeline', href: '/csr/pipeline/commercial', icon: <Briefcase size={24} /> },
        { label: 'Commercial Renewal', href: '/csr/renewals/commercial', icon: <RefreshCw size={24} /> },
        { label: 'Reports', href: '/csr/reports', icon: <FileText size={24} /> },
    ]

    const adminMenu: MenuItem[] = [
        { label: 'Dashboard', href: '/admin', icon: <LayoutGrid size={24} /> },
        { label: 'All Leads', href: '/admin/leads', icon: <GitBranch size={24} /> },
        { label: 'Lead Assignments', href: '/admin/assignments', icon: <ListTodo size={24} /> },
        { label: 'Pipelines Monitor', href: '/admin/pipelines', icon: <Activity size={24} /> },
        { label: 'CSR Performance', href: '/admin/csrs', icon: <Users size={24} /> },
        { label: 'Reports', href: '/admin/reports', icon: <PieChart size={24} /> },
    ]

    const accountingMenu: MenuItem[] = [
        { label: 'Dashboard', href: '/accounting', icon: <LayoutGrid size={24} /> },
        { label: 'Financial Reports', href: '/accounting/reports', icon: <DollarSign size={24} /> },
    ]

    const superadminMenu: MenuItem[] = [
        { label: 'Dashboard', href: '/superadmin', icon: <LayoutGrid size={24} /> },
        { label: 'Users', href: '/superadmin/users', icon: <Users size={24} /> },
        { label: 'Roles', href: '/superadmin/roles', icon: <Users size={24} /> },
        { label: 'Pipelines', href: '/superadmin/pipelines', icon: <Activity size={24} /> },
        { label: 'Email Templates', href: '/superadmin/email-templates', icon: <FileText size={24} /> },
        { label: 'Forms', href: '/superadmin/forms', icon: <ListTodo size={24} /> },
        { label: 'Settings', href: '/superadmin/system-settings', icon: <Settings size={24} /> },
        { label: 'Audit Logs', href: '/superadmin/audit-logs', icon: <Activity size={24} /> },
        { label: 'Admin View', href: '/admin', icon: <PieChart size={24} /> },
        { label: 'Accounting View', href: '/accounting', icon: <DollarSign size={24} /> },
    ]

    const getMenuForRole = () => {
        switch (role) {
            case 'csr': return csrMenu
            case 'admin': return adminMenu
            case 'accounting': return accountingMenu
            case 'superadmin': return superadminMenu
            default: return []
        }
    }

    const currentMenu = getMenuForRole()

    return (
        <aside
            className={`
                fixed left-0 bottom-0 z-[50] bg-gradient-to-b from-[#10B889] to-[#2E5C85] text-white flex flex-col shadow-xl 
                transition-all duration-300 ease-in-out
                top-16 lg:top-24
                ${isMobileOpen ? 'translate-x-0 w-[280px]' : '-translate-x-full lg:translate-x-0'}
                ${isHovered ? 'lg:w-[260px] items-start' : 'lg:w-[110px] items-center'}
            `}
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
        >
            <nav className="flex-1 flex flex-col gap-2 mt-3 w-full px-2 overflow-y-auto pb-8">
                {currentMenu.map((item, index) => (
                    <Link 
                        key={index} 
                        href={item.href} 
                        className="w-full"
                        onClick={() => setIsMobileOpen(false)}
                    >
                        <SidebarIcon
                            icon={item.icon}
                            label={item.label}
                            active={isActive(item.href)}
                            expanded={isHovered || isMobileOpen}
                        />
                    </Link>
                ))}
            </nav>
        </aside>
    )
}

function SidebarIcon({
    icon,
    label,
    active,
    expanded,
}: {
    icon: React.ReactNode
    label: string
    active?: boolean
    expanded: boolean
}) {
    return (
        <div
            title={label}
            className={`
                flex transition-all duration-300 ease-in-out rounded-xl cursor-pointer
                ${expanded
                    ? 'flex-row items-center justify-start h-[56px] px-4 gap-3 w-full'
                    : 'flex-col items-center justify-center h-[72px] w-[72px] gap-1.5 mx-auto'
                }
                ${active
                    ? 'bg-white text-[#10B889] shadow-lg'
                    : 'text-white/80 hover:bg-white/10 hover:text-white hover:shadow-md'
                }
            `}
        >
            <div className="flex-shrink-0">{icon}</div>
            <span
                className={`
                    font-semibold tracking-wide transition-all duration-300 whitespace-nowrap
                    ${expanded ? 'text-sm opacity-100' : 'text-[0px] opacity-0 overflow-hidden'}
                `}
            >
                {label}
            </span>
        </div>
    )
}